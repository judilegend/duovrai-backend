...
# Initialize Stripe API Key using the secret key expected by the Stripe SDK.
...
class StripeService:
    ...
    def is_mock_enabled() -> bool:
        secret_key = (settings.STRIPE_SECRET_KEY or settings.STRIPE_API_KEY or "").strip()
        return (
            not secret_key
            or secret_key.startswith("sk_test_mock")
            or secret_key.startswith("pk_test_")
            or secret_key.startswith("pk_live_")
        )
    ...
    def create_checkout_session(self, order: Order) -> dict:
        ...
        try:
            ...
            # Detect whether the Stripe price is one-time or recurring
            price = stripe.Price.retrieve(price_id)
            checkout_mode = "payment" if price.type == "one_time" else "subscription"

            # Create checkout session with correct mode for the Price type
            session_payload = {
